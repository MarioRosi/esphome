/*
 *   Eine eigene Rollershutter-Karte nach dem Vorbild von 
 *   https://github.com/Deejayfool/hass-shutter-card/blob/master/README.md
 *   https://github.com/Deejayfool/hass-shutter-card/blob/master/hass-shutter-card.js  
 * 
 *   So umgebaut, dass es aus dem Icon(==Status-Anzeige), dem Namen, den Buttons [Up,Stop,Gap,Down], und der Statustext Anzeige besteht.
 * 
 *   Mario Rosenbohm
 *   2025.06.15
 */
...
class RollerShutterCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    if (!this.card){
      const _card = document.createElement('ha-card');
      if (this.config.title) {
        _card.header = this.config.title;
      }
      this.card = _card;
      this.appendChild(_card);

      let allShutters = document.createElement('div');
      allShutters.className = 'rl-shutters';
  ...
  setConfig(config){
    if (!config.entities)
    {
      throw new Error('Your must define entities');
    }
    this.config = config;
    this.maxPosition = 50;
    this.minPosition = 50;
...
customElements.define("rollershutter-card", RollerShutterCard);